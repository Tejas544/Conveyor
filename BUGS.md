...
- **Severity:** Critical
- **Symptom:** `conveyor-verifier` flagged one order as a 3-invariant violation: `INV-ORD-03` (payment still `CAPTURED` on a `CANCELLED` order), `INV-DSP-01` (a `CANCELLED` order had a real shipment), and `INV-SAGA-05` (`sagaState=COMPLETED` but `orderStatus=CANCELLED`). The order's own saga step log showed `RESERVE_INVENTORY` marked `TIMED_OUT` at 00:16:40.558, immediately followed by the *same* step marked `SUCCEEDED` at 00:16:40.756 — 200ms later — after which the saga proceeded through `CHARGE_PAYMENT` and `CONFIRM_ORDER` to a real `COMPLETED` state (customer genuinely charged, a real shipment created), while `order-service`'s own `orders.status` — flipped to `CANCELLED` directly by the timeout sweep's own `OrderCancelled` publish — never learned any of that happened.
- **Root cause:** Unlike BUG-0026/0027/0036 (a late reply arriving *after* a saga had already reached a stable `ABORTED`/compensating state, previously fixed), this is a genuine intra-transaction race: `SagaTimeoutSweeper.sweepOnce()` claims a saga with a real Postgres row lock (`SELECT ... FOR UPDATE SKIP LOCKED`) and holds it for the whole `applyTimeoutPolicy` transaction (mark `TIMED_OUT`, set state `ABORTED`, publish `OrderCancelled`) — but `SagaOrchestrationService.requireSaga()` (used by every Kafka reply handler, including `handleInventoryReserved`) read the saga via a plain, unlocked `findByOrderId`. Under Postgres's MVCC, a late reply arriving *while the sweep's transaction was still open* (not yet committed) could read the pre-timeout `RESERVING_INVENTORY` snapshot, decide the saga was healthy, and — once its own write finally went through after the sweep committed (Postgres still serializes the actual `UPDATE`, just later than the read) — silently overwrite the sweep's `ABORTED` transition with the happy path's own state, with no re-check that its original read was still valid. `SagaInstance` carries no `@Version` field, so there was nothing to catch a stale write at commit time either. This is exactly the kind of window only real concurrent load can open: it needs a reply to land within roughly the same few hundred milliseconds as the sweep's own claim-and-process cycle for this saga, which Phase 15's own 60-VU confirmation run — with `SagaReplyListener` still single-threaded (Phase 12's own unresolved finding) — was finally heavy enough to produce.
- **Fix:** Added `SagaInstanceRepository.findByOrderIdForUpdate` (`@Lock(LockModeType.PESSIMISTIC_WRITE)`, a real blocking `SELECT ... FOR UPDATE`) and pointed `requireSaga()` at it. Every reply handler now blocks against a concurrently in-flight sweep transaction on the same row instead of reading around it, and always observes post-commit truth — the existing "saga already `ABORTED`" branches in each reply handler (BUG-0026/27/36's own fixes) were already correct, they simply never ran for this specific race because the read was stale. New regression test, `SagaTimeoutIntegrationTest#lateInventoryReservedRacingAConcurrentTimeoutSweepNeverClobbersTheAbort`: holds a real sweep transaction open for a fixed 1.5s window (via `TransactionTemplate`, not a signal from the reply thread — a signal fired from inside the reply call proved unreliable, since the whole point being tested is that an unlocked read never blocks at all) while a concurrent reply handler call attempts to run; verified failing without the fix (either the intended assertion or, in one run, a `saga_steps` unique-constraint violation from both transactions independently computing the same next sequence number — both are the same underlying race surfacing two different ways) and passing with it. Full `saga-orchestrator` suite green after (89/89, including the new test).
- **Status:** Fixed and verified via the new deterministic regression test (failing without the fix, passing with it) plus the full `saga-orchestrator` suite (29/29) and full reactor `./mvnw verify`, both green. Deliberately **not** re-verified against a second live 60-VU load run against a rebuilt image — the race is a few-hundred-millisecond window that a live re-run might simply not hit again, proving nothing either way, and this session had already spent considerable time recovering this same host's kind cluster from unrelated instability (CONTEXT.md). The one affected order from this session's own load test exists only on this phase's own ephemeral kind cluster (recreated multiple times this session, see CONTEXT.md); no separate data remediation needed beyond the fix itself.

---

## [BUG-0048] Phase 15's own instrumentation script crashed outright on one slow `kubectl top` call under real load
- **Date:** 2026-09-11
- **Phase:** Phase 15 — Autoscaling measurement (found live, first real ramp run against the kind cluster)
- **Severity:** Low
- **Symptom:** `scaling/watch.py`, launched to run unattended for ~18 minutes across the whole k6 ramp scenario plus a post-load scale-down observation window, died with an uncaught `subprocess.TimeoutExpired` after roughly 12 minutes — losing every remaining tick, including the entire scale-down half of the run, the half that mattered just as much as scale-up.
- **Root cause:** `kubectl top pods` (metrics-server) took longer than the script's fixed 15s timeout on at least one poll, under the same real CPU pressure this phase's own load test and replica fan-out were generating — exactly the kind of transient, load-induced slowness this project has hit before in other tools (BUG-0044), but this script had no retry/skip handling for it at all.
- **Fix:** Every per-tick `kubectl` call (`kubectl_json`, `kubectl_top_pods`) now catches `subprocess.TimeoutExpired` and returns "no data for this tick" instead of propagating, so one slow poll costs one data point, not the rest of the run; timeout raised 15s -> 20s as well.
- **Status:** Fixed.

---

## [BUG-0047] `saga-orchestrator`'s startup probe crash-looped every replica added by its own new HPA, indefinitely, under real per-pod CPU-limit throttling
- **Date:** 2026-09-11
- **Phase:** Phase 15 — Autoscaling measurement (found live, the same ramp run that triggered the custom-metric HPA's first-ever real scale-up)
- **Severity:** High
- **Symptom:** As `saga-orchestrator`'s HPA correctly scaled 1 -> 6 replicas in response to a real `conveyor_saga_active` backlog, every new replica beyond the first cycled `Running` (0/1) -> killed by a failed startup probe -> restarted, repeatedly, for as long as observed (multiple restarts over 8+ minutes, never once reaching `Ready`) — while cluster-wide node CPU utilization stayed at a moderate 33% the whole time (`kubectl top nodes`), ruling out node-level exhaustion as the cause.
- **Root cause:** `infra/helm/conveyor/templates/_helpers.tpl`'s shared `startupProbe` (`periodSeconds: 2`, `failureThreshold: 45` = 90s total) was sized for one cold single-replica start, not several sibling JVMs starting at once, each throttled by its own `resources.limits.cpu` (`saga-orchestrator`: 1000m, i.e. 1 core). A live log of one such pod showed Spring's `Root WebApplicationContext` alone taking 31 seconds to initialize (normally single-digit seconds) — Spring context startup is genuinely CPU-bound, and a cgroup CPU *limit* throttles it the same way genuine host contention would, even with headroom free elsewhere on the node. Every restart reset the 90s countdown to zero, so a pod that consistently needed more than 90s to finish never got a chance to.
- **Fix:** `failureThreshold` raised 45 -> 90 (90s -> 180s tolerance) for the shared probe template, applied to every service, not just `saga-orchestrator` (the same fan-out risk applies to any of them under a future HPA). Verified live: after `helm upgrade`, all 6 `saga-orchestrator` replicas reached `Running`/`1/1` with no further restarts.
- **Status:** Fixed. Named, not hidden, in `RESULTS.md`'s Phase 15 section as a real component of scale-up latency under concurrent replica fan-out — the CPU-limit-throttling cost is a genuine measured finding, not just an artifact to fix away.

---

## [BUG-0046] Postgres's default `max_connections=100` left almost no headroom once order-service's HPA can scale to 5 replicas
- **Date:** 2026-09-11
- **Phase:** Phase 15 — Autoscaling measurement (found live, resuming Phase 13/14's kind cluster after it had been running unattended for ~9 hours)
- **Severity:** Medium
- **Symptom:** A `conveyor-verifier` CronJob run failed with `HikariPool$PoolInitializationException: FATAL: remaining connection slots are reserved for roles with the SUPERUSER attribute` — five consecutive runs over roughly 15 minutes errored the same way before self-resolving.
- **Root cause:** `infra/helm/conveyor/templates/postgres/statefulset.yaml` never set `max_connections`, leaving Postgres's own default of 100. Five single-replica app services at Spring Boot's default Hikari pool size of 10 each already account for 50; `conveyor-verifier`'s once-a-minute oneshot run opens 5 more (one per service database) on top of that. There was already very little headroom before Phase 15 existed — and Phase 15's own point is to scale `order-service` to as many as 5 replicas (5×10 = 50 by itself) and `saga-orchestrator` to as many as 6 (6×10 = 60), which would have pushed sustained connection demand well past 100 during the very load test this phase needs to run, misattributing a capacity ceiling this project itself introduced to a saga/inventory/payment defect it does not have.
- **Fix:** `args: ["postgres", "-c", "max_connections=300"]` added to the Postgres container spec — headroom for this phase's own maximum possible replica fan-out, not a per-connection cost concern on a local dev host.
- **Status:** Fixed (verified live: `SHOW max_connections` reports 300 after `helm upgrade`, all app pods restarted healthy).
