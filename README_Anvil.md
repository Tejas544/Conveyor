...
seeded-mutation report ............... every deliberate bug in the tree, one
  ...
  must-detect caught ................. 19/23 (was 23/23 before ANV-0067 part 1
                                       landed, 22/23 immediately after it, and
                                       19/23 after ANV-0067 part 4, the dual-
                                       copy hard-state write -- see below)
  invisible from the client API ...... 6/19
                                       Six bugs that were caught and that no
  ...
  controls silent .................... 9/10 (1 fired: "no commit-wait")
  equivalent-in-class ................ 2, each with a written argument
  covered by a named test ............ 1
                                       Every miss and every newly-firing
                                       control was checked head-to-head
                                       against a pre-fix binary before being
                                       reported here: one miss ("intents
                                       invisible to readers") predates this
                                       session entirely; the other three
                                       ("commit across terms", "uncertain
                                       reads never restart", "uncertainty
                                       never honoured") were already at
                                       single-digit-out-of-15 detection rates
                                       and are a direct, expected consequence
                                       of every hard-state write now costing
                                       two files and two fsyncs instead of
                                       one, which reshuffles every downstream
                                       schedule (CONTEXT.md 5a.3, 10.34) --
                                       not a new logic defect in any of the
                                       mutated code paths, none of which this
                                       session's change touched. Full argument
                                       in BUGS.md ANV-0067/ANV-0032.
...
seed fleet ........................... 5 workloads/seed, one shard per core,
  ...
  sharpest new one at the time ........ ANV-0066, minimised from the 11 fault
                                       features its seed drew to 2 (partition +
                                       disk.slow_io), 1-minimality verified in
                                       45 runs -- update: re-run standalone
                                       during the 2026-09-06 fleet-backlog
                                       triage, it no longer reproduces its own
                                       documented result against current HEAD.
                                       See BUGS.md ANV-0066's `update` field
  honest caveat ...................... the fleet classifies more narrowly than
                                       each suite does, so "7 new" is an
                                       over-approximation and a triage queue.
                                       Two later fleet passes (12,897 runs)
                                       raised that queue to 80 classes; a
                                       2026-09-06 triage found most of the
                                       non-mvcc, non-bit-rot ones do not
                                       reproduce standalone either -- see
                                       CLAUDE.md's P8 criterion 1 and BUGS.md
                                       ANV-0070 through ANV-0073

bugs found (S0/S1/S2/S3/S4) .......... 24/14/11/2/20 + 2 unclassified
                                       (see BUGS.md; 10 open -- recount as of
                                       the 2026-09-06 fleet-backlog triage,
                                       which added ANV-0070 through ANV-0073
                                       and reopened the question on ANV-0066)
